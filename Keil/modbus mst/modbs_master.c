...
void mbInit(lpMbParam_t mbParam)
{
		...
		float end_packet_delay = 4000000/baudrate;					// +/- время на 4 символа в микросекундах
...
void mb_Timer_Int( lpMbParam_t mbParam )
{
		...
		if((mbParam->timeLeft < mbParam->timeOut) && (mbParam->Flags.enableTimer == 1) && (!mbParam->Flags.isHostTimeout))
		{
...
void mbParsePacket( lpMbParam_t mbParam )
{
		...
		if( (mbParam->function == 0x03) && (mbParam->rxCount - 1 == ((mbParam->num_regs)*2 + 5)))				// если читаем 0x03, то отает должен быть адрес(1) + функция(1) + количество(1) + CRC(2) + (num_regs*2)		байт
		{	
				uint16_t checkCRC = CRC16_mbm((unsigned char *)(mbParam->rxBuf), (mbParam->rxCount) - 1);
